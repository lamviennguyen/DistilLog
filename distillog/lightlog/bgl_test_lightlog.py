...
'''
split = 50
path = "../datasets/BGL/random_test.csv"

ppa_result = pd.read_csv("../datasets/BGL/bgl_semantic_vector.csv")
...
def read_test(i):
    ...
    for i in range(0, len(logs_data)):
        padding = np.zeros((50, 20))
        data = logs_data[i]
        data = [int(n) for n in data.split()]
        for j in range(0, len(data)):
            padding[j] = ppa_result[data[j]-1]
        padding = list(padding)
        logs.append(padding)
    logs = np.array(logs)
    test_x = logs
    test_y = label
    test_x = np.reshape(test_x, (test_x.shape[0], test_x.shape[1], 20))
    #test_y = keras.utils.np_utils.to_categorical(np.array(test_y))
...
def test_model(test_x, label):
    model = TCN()
    model.to(device)
    model.load_state_dict(torch.load("../datasets/BGL/model/random_lightlog.pth",map_location=torch.device('cpu')))
    model.eval()
